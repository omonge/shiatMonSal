...
<title>ChronoMananger - Listar Contactos</title>
...
<!-- fwtable fwsrc="banner_ctp_rh.png" fwpage="Page 1" fwbase="plantilla.png" fwstyle="Dreamweaver" fwdocid = "1063142010" fwnested="0" -->
  ...
  <tr>
   ...
   <!-- InstanceBeginEditable name="titulo" --><h:outputText styleClass="outputText" value="#{rcs['titulo.contactoConsulta']}"/><!-- InstanceEndEditable --></h2>
  ...
  <tr height="100%">
   ...
   <td colspan="7" valign="top" height="100%"><!-- InstanceBeginEditable name="contenido" -->
   		...
   		<tbody>
   			<tr>   				
				...
				<td class="odd"><h:inputText styleClass="inputText"
					id="txtBuscar"
					value="#{contactoController.criterioBusqueda}"></h:inputText>
	...
	<h:commandButton type="submit"
				...
				action="#{contactoController.buscarPorCodigo}">
	</h:commandButton>
	<h:commandButton type="submit"
				value="#{rcs['buscar.boton.descripcion']}"
				styleClass="commandButton" id="btnBuscarNombre"
				action="#{contactoController.buscarPorDescripcion}">
	</h:commandButton>
	<br/>
   ...
   <h:dataTable border="1" columnClasses="" headerClass="Tit04Tabla"
				...
				value="#{contactoController.contactos}" var="contactos" rows="20" 
				binding="#{contactoController.dataTableContactos}">		
		<h:column id="Columna1">
			<f:facet name="header">
				<h:outputText styleClass="outputText"
					value="#{rcs['contacto.codigo']}"
					id="lbl1"></h:outputText>
			</f:facet>
				<h:commandLink styleClass="commandLink" id="link1" action="#{contactoController.cargarObjeto}">
					<h:outputText styleClass="outputText" id="text1"
						value="#{contactos.pvLoCodigo}">
					</h:outputText>
		...
		<h:column id="Columna7">
			<f:facet name="header">
				<h:outputText styleClass="outputText"
					value="#{rcs['contacto.detalles']}"
					id="lbl7"></h:outputText>
			</f:facet>		
			<jp:popupFrame id="popUpDetalleContacto" width="650px" height="400px" 
	          	popupTitle="#{rcs['contactoPop.titulo']}"
	          	title="#{rcs['contactoPop.titulo']}"
	          	actionOpen="#{contactoController.abrirPopUpContacto}"
	          	actionClose="#{contactoController.cerrarPopUpContacto}" immediate="true" center="true">
				<h:graphicImage styleClass="graphicImage" id="imgDetalle" value="images/ver_det.gif" height="16" width="80" style="icono"></h:graphicImage>
			</jp:popupFrame>
		</h:column>
	</h:dataTable>
	...
	<p>
		<h:commandButton type="submit" value="#{rcs['boton.agregar']}" styleClass="commandButton" id="button1" action="#{contactoController.irAgregar}"></h:commandButton>
	</p>
	</h:form>	
