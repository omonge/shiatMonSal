...
public class ProfesionController  extends Controller{

	private HtmlDataTable     listaDataTable;
    private List<Profesion>   lista;
	...
	protected void reiniciarDatos() {
		...
		this.lista = new ArrayList<Profesion>();
	}
    ...
    private boolean validarFiltro() {
    	...
    	if ((this.profesion.getPvStDescripcion() != null) && (!"".equals(this.profesion.getPvStDescripcion()))) {
            correcto = true;
    	}
        return correcto;
    }
    ...
    /**
     * Método buscar Busca las listaprofesion que cumplan con el filtro
    ...
    public String buscar() {
        ...
        if (this.validarObjetoId()) {
            Profesion profesion = this.profesionBo.buscar(this.profesion);
            if (profesion != null) {
                this.lista.add(profesion);
            }
            respuesta = "success";
        } else{
        	if (this.validarFiltro()) {
        		this.lista = this.profesionBo.getProfesion(this.profesion);
        		respuesta = "success";
        	}
            else{ 
	...
	public void buscarTodos(){
		this.lista = this.profesionBo.getProfesion();
	...
	*/
	public String cargarObjeto(){
		 this.profesion = (Profesion) this.listaDataTable.getRowData();
	     this.agregar = false;
		 return "success";
	}
	...
	public String insertar(){
        ...
        try{
            if(this.validaInsertar()){
            	this.profesion.setPvStEstado(Profesion.ESTADO_ACTIVO);
	...
	*/
	public String modificar(){
        ...
        try{
            if(this.validaInsertar()){
                this.profesionBo.modificar(this.profesion);
	...
	/**Método eliminar
	* Eliminar un profesion en la base de datos
	*@return success si logra eliminar, error en caso contrario
	*/
	public String eliminar(){
		String respuesta = "error";
        try{
            this.profesionBo.eliminar(this.profesion);
	...
	protected String getPropertyFieldName(String property) {
		if(property != null){
			if (property.equals("profesion.pvInCodigo")) return "form1:pvInCodigo";
			if (property.equals("profesion.pvStDescripcion")) return "form1:pvStDescripcion";
		}
		return null;
	}
	 ...
	 * @return the lista
	 */
	public List<Profesion> getLista() {
		return lista;
	}
	 ...
	 */
	public void setLista(List<Profesion> lista) {
		this.lista = lista;
	}
